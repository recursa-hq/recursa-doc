...
describe('MCP Server HTTP Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const mockHandleUserQuery = mock(async (query: string, config: AppConfig, sessionId?: string) => {
  ...
  describe('HTTP API Endpoints', () => {
    ...
    it('should handle POST /mcp endpoint with valid request', async () => {
      ...
      const response = await app.handle(new Request('http://localhost:3001/mcp', {
        method: 'POST',
        headers: {
      ...
      expect(data.reply).toContain('Mock response for query: Create a test file');
  ...
  describe('Server-Sent Events (SSE)', () => {
    it('should establish SSE connection for valid runId', async () => {
      ...
      const response = await app.handle(new Request(`http://localhost:3001/events/${testRunId}`));
    ...
    it('should send initial connection message via SSE', async () => {
      ...
      if (reader) {
        ...
        if (dataMatch) {
          ...
          expect(initialMessage).toHaveProperty('content', 'Connection established');
        }
      }
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle errors in MCP processing gracefully', async () => {
      ...
      const response = await errorApp.handle(new Request('http://localhost:3001/mcp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      expect(data).toHaveProperty('runId');
      expect(data).toHaveProperty('reply');
      expect(data.reply).toContain('An error occurred while processing your request.');
    });

    it('should handle 404 for unknown routes', async () => {
      const response = await app.handle(new Request('http://localhost:3001/unknown-route'));
      expect(response.status).toBe(404);
    });

    it('should reject unsupported HTTP methods for /mcp endpoint', async () => {
      // GET method should be rejected
      const getResponse = await app.handle(new Request('http://localhost:3001/mcp', {
        method: 'GET',
      }));
      expect(getResponse.status).toBe(404); // Route not found for GET

      // PUT method should be rejected
      const putResponse = await app.handle(new Request('http://localhost:3001/mcp', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const deleteResponse = await app.handle(new Request('http://localhost:3001/mcp', {
        method: 'DELETE',
      }));
