...
export interface MemAPI {
  ...
  updateFile: (filePath: string, oldContent: string, newContent: string) => Promise<boolean>;
  ...
  gitDiff: (filePath: string, fromCommit?: string, toCommit?: string) => Promise<string>;
  ...
  getFilesByTokenSize: (directoryPath?: string, limit?: number) => Promise<Array<{filePath: string; tokens: number}>>;
  getDirectoryTokenStats: (directoryPath?: string) => Promise<DirectoryTokenStats>;
}
